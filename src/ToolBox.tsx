...
class ToolBox extends React.Component<Props, States> {
  ...
  handleSelectedToolChange = (tool: Tool) => this.props.onSelectedToolChange(tool);
  ...
  render() {
    return (
      <div className={'tool-box'}>
        <button
          onClick={() => this.handleSelectedToolChange(this.props.tools.getById('pen'))}
          className={(this.props.selectedTool.displayName === 'ペン' ? 'pen-btn active' : 'pen-btn')}>
          ペン
        </button>
        <button
          onClick={() => this.handleSelectedToolChange(this.props.tools.getById('eraser'))}
          className={(this.props.selectedTool.displayName === '消しゴム' ? 'eraser-btn active' : 'eraser-btn')}>
          消しゴム
        </button>
        ...
        <div>
          <div>ペンの太さ: {this.props.tools.getById('pen')?.thicknessCoefficient}</div>
          <input onChange={e => this.penThicknessChange(Number(e.target.value))} defaultValue={this.props.tools.getById('pen').thicknessCoefficient} type="range" min="8" max="256"></input>
        </div>
        
        <div>
          <div>消しゴムの太さ: {this.props.tools.getById('eraser')?.thicknessCoefficient}</div>
          <input onChange={e => this.eraserThicknessChange(Number(e.target.value))} defaultValue={this.props.tools.getById('eraser').thicknessCoefficient} type="range" min="8" max="256"></input>
        </div>
      </div>
    );
  }
