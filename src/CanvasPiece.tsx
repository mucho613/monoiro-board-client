...
class CanvasPiece extends React.Component<Props> {
  ...
  commit = (action: any) => {
    ...
    this.canvasContext.globalCompositeOperation = action.tool.type === ToolType.Pen
        ? 'source-over'
        : 'destination-out';
    ...
    if(action.image) {
      ...
      this.canvasContext.drawImage(this.temporaryCanvas, action.left, action.top);
    }
  }

  clear = () => {
    if(!this.canvasContext) throw new Error('Canvas Context が利用できません');
    this.canvasContext.clearRect(0, 0, this.props.canvasWidth, this.props.canvasHeight);
  }
